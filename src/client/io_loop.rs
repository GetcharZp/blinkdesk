        ...
        .await
        {
            Ok(((mut peer, direct, pk, kcp, stream_type), (feedback, rendezvous_server))) => {
                ...
                let mut status_timer =
                    crate::blinkdesk_interval(time::interval(Duration::new(1, 0)));
                    ...
                    tokio::select! {
                        ...
                        _ = self.timer.tick() => {
                            ...
                            if !self.read_jobs.is_empty() {
                                ...
                                self.timer = crate::blinkdesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                            }
                        }
                        _ = status_timer.tick() => {
                            if self.handler.is_restarting_remote_device()
                ...
                if is_remote {
                    ...
                    match fs::TransferJob::new_read(
                        ...
                        Ok(job) => {
                            ...
                            self.timer = crate::blinkdesk_interval(time::interval(MILLI1));
                            allow_err!(
