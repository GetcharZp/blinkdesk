...
impl<T: InvokeUiCM> IpcTaskRunner<T> {
    async fn run(&mut self) {
        ...
        let mut file_timer =
            crate::blinkdesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
            ...
            tokio::select! {
                            ...
                            match data {
                                ...
                                Data::FS(mut fs) => {
                                    ...
                                    if !self.read_jobs.is_empty() {
                                        file_timer = crate::blinkdesk_interval(time::interval(MILLI5));
                                    }
                ...
                _ = file_timer.tick() => {
                    if !self.read_jobs.is_empty() {
                        ...
                        file_timer = crate::blinkdesk_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
            }
        }
