...
pub fn portable_service_logon_helper_paths() -> Option<(PathBuf, PathBuf)> {
    ...
    let dir = user_dir
        ...
        .join("AppData")
        .join("Local")
        .join("blinkdesk-sciter");
    let dst = dir.join("blinkdesk.exe");
    Some((dir, dst))
}
...
pub fn install_me(options: &str, path: String, silent: bool, debug: bool) -> ResultType<()> {
    ...
    let tmp_path = "%BLINKDESK_OUTPUT_DIR%".to_owned();
...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("BlinkDesk")
        .join("BlinkDeskCustomClientStaging")
}
...
/// - Deleting directories under a public location (e.g., C:\\ProgramData\\BlinkDesk) is
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging BlinkDesk.
...
// caller's own context, so installing or toggling the service from a BlinkDesk
...
taskkill /F /IM {app_name}.exe{filter}
{tray_shortcut_commands}
copy /Y \"%BLINKDESK_OUTPUT_DIR%\\{app_name} Tray.lnk\" \"%PROGRAMDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\\"
{import_config}
{create_service}
    ",
        import_config = get_import_config(exe),
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `blinkdesk --connect` are running.
...
fn build_msi_uninstall_command(product_code: &str) -> String {
    format!(
        "set \"BLINKDESK_MSI_EXIT_CODE=\"\n\
MsiExec.exe /X {product_code} /norestart REBOOT=ReallySuppress\n\
set \"BLINKDESK_MSI_EXIT_CODE=%ERRORLEVEL%\"\n\
if \"%BLINKDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_REQUIRED}\" echo MSI uninstall succeeded with a reboot recommendation; continuing without reboot.\n\
if \"%BLINKDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_INITIATED}\" echo MSI uninstall succeeded with a reboot request; continuing without forcing reboot.\n\
if not \"%BLINKDESK_MSI_EXIT_CODE%\"==\"0\" if not \"%BLINKDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_REQUIRED}\" if not \"%BLINKDESK_MSI_EXIT_CODE%\"==\"{MSI_EXIT_SUCCESS_REBOOT_INITIATED}\" exit /b %BLINKDESK_MSI_EXIT_CODE%\n\
ver > nul"
    )
}
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like blinkdesk-*.msi or blinkdesk-*.exe
                if file_name.starts_with("blinkdesk-")
...
pub fn try_kill_blinkdesk_main_window_process() -> ResultType<()> {
    // Kill blinkdesk.exe without extra arg, should only be called by --server
    ...
    bail!("failed to find blinkdesk main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_blinkdesk.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\BlinkDesk\BlinkDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\BlinkDesk\BlinkDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
