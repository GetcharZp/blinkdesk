...
typedef char** (*FUNC_BLINKDESK_CORE_MAIN)(int*);
typedef void (*FUNC_BLINKDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_BLINKDESK_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_BLINKDESK_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("libblinkdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libblinkdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_BLINKDESK_CORE_MAIN blinkdesk_core_main =
      (FUNC_BLINKDESK_CORE_MAIN)GetProcAddress(hInstance, "blinkdesk_core_main_args");
  if (!blinkdesk_core_main)
  {
    std::cout << "Failed to get blinkdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_BLINKDESK_FREE_ARGS free_c_args =
      (FUNC_BLINKDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = blinkdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "BlinkDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);
  FUNC_BLINKDESK_IS_DISABLE_INSTALLATION blinkdesk_is_disable_installation =
      (FUNC_BLINKDESK_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "blinkdesk_is_disable_installation");
  bool is_disable_installation =
      blinkdesk_is_disable_installation && blinkdesk_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"BlinkDesk";
  FUNC_BLINKDESK_GET_APP_NAME get_blinkdesk_app_name = (FUNC_BLINKDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_blinkdesk_app_name");
  if (get_blinkdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_blinkdesk_app_name(app_name_buffer, 512) == 0) {
