...
#define BLINKDESK_LIB_PATH "libblinkdesk.so"
typedef bool (*BlinkDeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

// The bundle keeps the core library at lib/libblinkdesk.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  char lib_path[PATH_MAX + sizeof("/lib/" BLINKDESK_LIB_PATH)];
  snprintf(lib_path, sizeof(lib_path), "%s/lib/%s", exe_path, BLINKDESK_LIB_PATH);
  if (access(lib_path, F_OK) != 0) return nullptr;
  void* core_lib = dlopen(lib_path, RTLD_LAZY);
  if (!core_lib) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_blinkdesk_core_main() {
   void* core_lib = dlopen_bundled_lib();
   if (!core_lib) {
      core_lib = dlopen(BLINKDESK_LIB_PATH, RTLD_LAZY);
   }
   if (!core_lib) {
      fprintf(stderr,"Failed to load \"libblinkdesk.so\"\n");
      char* error;
   ...
   auto core_main = (BlinkDeskCoreMain) dlsym(core_lib,"blinkdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"blinkdesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_blinkdesk_core_main()) {
      return 0;
  }
