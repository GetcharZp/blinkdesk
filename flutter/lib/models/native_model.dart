...
/// The Linux bundle keeps the core library at lib/libblinkdesk.so next to the
...
DynamicLibrary _openLinuxCoreLib() {
  final bundled =
      '${File(Platform.resolvedExecutable).parent.path}/lib/libblinkdesk.so';
  try {
  ...
  return DynamicLibrary.open('libblinkdesk.so');
}
...
class PlatformFFI {
  ...
  BlinkdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libblinkdesk.so')
        : isLinux
            ? _openLinuxCoreLib()
            : isWindows
                ? DynamicLibrary.open('libblinkdesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("libblinkdesk.dylib") :
  ...
  void _startListenEvent(BlinkdeskImpl blinkdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = blinkdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
