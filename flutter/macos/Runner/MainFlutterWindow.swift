...
class MainFlutterWindow: NSWindow {
    ...
    private func enableNativeRelativeMouseMode(channel: FlutterMethodChannel) -> Bool {
        ...
        if result != CGError.success {
            NSLog("[BlinkDesk] Failed to dissociate mouse from cursor position: %d", result.rawValue)
            return false
        }
        ...
        if state.eventMonitor == nil {
            NSLog("[BlinkDesk] Failed to create event monitor for relative mouse mode")
    ...
    private func disableNativeRelativeMouseMode() {
        ...
        if result != CGError.success {
            NSLog("[BlinkDesk] Failed to re-associate mouse with cursor position: %d, scheduling retry...", result.rawValue)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                if retryResult != CGError.success {
                    NSLog("[BlinkDesk] Retry failed to re-associate mouse: %d. Cursor may remain locked.", retryResult.rawValue)
                }
            }
        }
    }
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "org.blinkdesk.blinkdesk/host", binaryMessenger: registrar.messenger)
