...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("BlinkDesk".to_owned());
    ...
    // Local SOCKS endpoint of the currently running SSH tunnel.
    // Runtime-only override, not persisted. Set by the SSH tunnel manager in
    // the app (`src/ssh_tunnel.rs`) so that all outbound TCP traffic is routed
    // through the tunnel with the same machinery as a socks5 proxy.
    static ref SSH_TUNNEL_ACTIVE_SOCKS: RwLock<Option<Socks5Server>> = RwLock::new(None);
}
...
pub const DEFAULT_SSH_PORT: i32 = 22;

/// An SSH tunnel (dynamic SOCKS forwarding over an SSH connection, the
/// `ssh -D` equivalent, implemented with the russh client).
///
/// When set, all outbound TCP traffic is routed through a local SOCKS
/// endpoint created by the tunnel manager (`src/ssh_tunnel.rs`), similar to
/// how a socks5 proxy routes traffic.
#[derive(Debug, Default, PartialEq, Serialize, Deserialize, Clone)]
pub struct SshTunnel {
    /// SSH server host or IP.
    #[serde(default, deserialize_with = "deserialize_string")]
    pub host: String,
    /// SSH server port, 0 means [`DEFAULT_SSH_PORT`].
    ...
    #[serde(default, deserialize_with = "deserialize_string")]
    pub password: String,
    /// Path to a private key file (optional, preferred over password).
    #[serde(default, deserialize_with = "deserialize_string")]
...
impl Config {
    ...
    fn apply_permanent_password_storage_for_sync(
        ...
        if decode_permanent_password_h1_from_storage(storage).is_none() {
            // Legacy plaintext storage from an older config is still usable for
            // local auth but cannot be synced in the current format. Skip the
            // update instead of failing the periodic sync.
            log::debug!("Skipping non-current permanent password storage sync payload");
            return Ok(false);
        }
    ...
    pub fn get_socks() -> Option<Socks5Server> {
        if let Some(socks) = SSH_TUNNEL_ACTIVE_SOCKS.read().unwrap().clone() {
            return Some(socks);
        }
        Self::get_socks_configured()
    }

    #[inline]
    pub fn get_socks_configured() -> Option<Socks5Server> {
    ...
    pub fn set_ssh_tunnel(ssh_tunnel: Option<SshTunnel>) {
        let mut config = CONFIG2.write().unwrap();
        if config.ssh_tunnel == ssh_tunnel {
            return;
        }
    ...
    #[inline]
    pub fn get_ssh_tunnel() -> Option<SshTunnel> {
        CONFIG2.read().unwrap().ssh_tunnel.clone()
    }

    /// Runtime-only: record the local SOCKS endpoint served by the active SSH
    /// tunnel (`ssh -D` equivalent), or `None` when the tunnel is down.
    /// Not persisted; set by `src/ssh_tunnel.rs` in the process that owns the
    /// rendezvous mediator.
    pub fn set_ssh_tunnel_local_socks(socks: Option<Socks5Server>) {
        *SSH_TUNNEL_ACTIVE_SOCKS.write().unwrap() = socks;
    }

    #[inline]
    pub fn is_ssh_tunnel_active() -> bool {
        SSH_TUNNEL_ACTIVE_SOCKS.read().unwrap().is_some()
    }
    ...
    pub fn get_network_type() -> NetworkType {
        if SSH_TUNNEL_ACTIVE_SOCKS.read().unwrap().is_some() {
...
pub fn option2bool(option: &str, value: &str) -> bool {
        ...
        || option == "force-always-relay"
    {
        ...
        // `direct-server` falls here: enabled by default, disable with "N".
        value != "N"
    }
}
...
pub mod keys {
    ...
    pub const OPTION_LAN_PASSWORD: &str = "lan-password";
